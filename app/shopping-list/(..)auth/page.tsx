 ...
 *
 * Ten route przechwytuje nawigację z /shopping-list do /auth
 * i wyświetla modal zamiast pełnej strony.
 *
 * Wzorzec: (..)auth oznacza przechwycenie jeden poziom wyżej
 * (z app/shopping-list/ do app/(public)/auth/)
 */
...
export default async function AuthModalPage({ searchParams }: PageProps) {
  ...
  if (user && !authError) {
    ...
    if (!profile?.disclaimer_accepted_at) {
      ...
      // Use redirect param or default to shopping-list
      const redirectTo = params.redirect || '/shopping-list'
      redirect(redirectTo)
    }
  }
