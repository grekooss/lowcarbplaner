...
CREATE TABLE content.recipes (
  ...
  total_calories INTEGER, -- Zdenormalizowana wartość dla optymalizacji, aktualizowana przez trigger
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

COMMENT ON COLUMN content.recipes.instructions IS 'Instrukcje przygotowania w formacie JSONB.';
COMMENT ON COLUMN content.recipes.total_calories IS 'Zdenormalizowana suma kalorii ze składników bazowych, dla szybszego sortowania.';
...
2.  **Automatyzacja (Triggery)**:
    ...
    - **Denormalizacja Kalorii**: Trigger na tabeli `content.recipe_ingredients` będzie przeliczał i aktualizował pole `total_calories` w tabeli `content.recipes` po każdej zmianie składników, co znacząco optymalizuje funkcję wymiany posiłku.
