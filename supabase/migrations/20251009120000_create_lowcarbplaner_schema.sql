...
-- ============================================================
-- 5. Create content.ingredients table
-- ============================================================
...
comment on table content.ingredients is 'Master data for all available ingredients with nutritional values per 100 units.';
comment on column content.ingredients.category is 'Type-safe ingredient category (ENUM): vegetables, fruits, meat, fish, dairy, eggs, nuts_seeds, oils_fats, spices_herbs, flours, beverages, sweeteners, condiments, other.';
comment on column content.ingredients.image_url is 'Optional URL to ingredient image stored in Supabase Storage.';
...
-- ============================================================
-- 6. Create content.ingredient_unit_conversions table
-- ============================================================
...
comment on table content.ingredient_unit_conversions is 'Conversion table mapping custom units to grams for precise calculations.';
comment on column content.ingredient_unit_conversions.unit_name is 'Custom unit name (e.g., sztuka, plaster, łyżka).';
comment on column content.ingredient_unit_conversions.grams_equivalent is 'Equivalent weight in grams for the custom unit.';

-- ============================================================
-- 7. Create content.recipes table
-- ============================================================
...
comment on table content.recipes is 'Master data for all available recipes with denormalized nutritional totals.';
comment on column content.recipes.instructions is 'Recipe preparation steps in JSONB format: [{"step": 1, "description": "...", "imageUrl": "..."}].';
comment on column content.recipes.meal_types is 'Array of meal types this recipe is suitable for (breakfast, lunch, dinner).';
comment on column content.recipes.tags is 'Tags for filtering and categorization (e.g., kurczak, szybkie, patelnia).';
comment on column content.recipes.total_calories is 'Denormalized total calories calculated from base ingredients, updated automatically by trigger (US-013).';
comment on column content.recipes.total_protein_g is 'Denormalized total protein calculated from base ingredients, updated automatically by trigger (US-013).';
comment on column content.recipes.total_carbs_g is 'Denormalized total carbs calculated from base ingredients, updated automatically by trigger (US-013).';
comment on column content.recipes.total_fats_g is 'Denormalized total fats calculated from base ingredients, updated automatically by trigger (US-013).';
...
-- ============================================================
-- 8. Create content.recipe_ingredients table
-- ============================================================
...
comment on table content.recipe_ingredients is 'Many-to-many relationship linking recipes to ingredients with denormalized nutritional values.';
comment on column content.recipe_ingredients.base_amount is 'Base amount of ingredient in the specified unit.';
comment on column content.recipe_ingredients.is_scalable is 'Indicates if the meal generation algorithm can modify this ingredient amount.';
comment on column content.recipe_ingredients.step_number is 'Optional step number where this ingredient is used in the recipe.';
comment on column content.recipe_ingredients.calories is 'Denormalized calories for base_amount, calculated automatically by trigger (US-013).';
comment on column content.recipe_ingredients.protein_g is 'Denormalized protein for base_amount, calculated automatically by trigger (US-013).';
comment on column content.recipe_ingredients.carbs_g is 'Denormalized carbs for base_amount, calculated automatically by trigger (US-013).';
comment on column content.recipe_ingredients.fats_g is 'Denormalized fats for base_amount, calculated automatically by trigger (US-013).';
...
begin
  ...
  set
    total_calories = (
      ...
      from content.recipe_ingredients
...
create trigger recipe_total_nutrition_trigger
  after insert or update or delete on content.recipe_ingredients
  for each row
...
create table public.planned_meals (
  ...
  recipe_id bigint references content.recipes on delete set null, -- SET NULL to preserve meal history if recipe is deleted
...
comment on column public.planned_meals.recipe_id is 'Foreign key to content.recipes. Set to NULL if recipe is deleted to preserve meal history.';
...
--
-- Next steps:
-- 1. Seed content.ingredients table with base ingredient data
-- 2. Seed content.recipes table with base recipe data
